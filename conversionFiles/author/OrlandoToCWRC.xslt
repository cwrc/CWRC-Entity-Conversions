                ...
                <entity>
                    ...
                    <recordInfo>
                        ...
                        <xsl:if test="@PERSONTYPE">
                            <personTypes>
                                <personType>orl&#58;<xsl:value-of select="fn:normalize-space(@PERSONTYPE)"/>
                                </personType>
                            </personTypes>
                        </xsl:if>
                    ...
                    <description>
                        <xsl:analyze-string select="@STANDARD" regex="{$occupationCheck}">
                            <xsl:matching-substring>
                                ...
                                <xsl:choose>
                                    <xsl:when test="fn:matches($occupationVar, '^((the elder)|(the younger)|(fils)|(pere)|(father)|(mother)|(son)|(daughter)|(neé))$')">
                                    </xsl:when>
                                    <xsl:otherwise>
                                        <occupations>
                                            <occupation>
                        ...
                        <existDates>
                          <xsl:analyze-string select="@STANDARD" regex="m\. ?(by)? ?\d+">
                            <xsl:matching-substring>
                              <dateSingle>
                                <textDate>
                            ...
                            <xsl:analyze-string select="@STANDARD" regex="fl\. ?(by)? ?\d+">
                                <xsl:matching-substring>
                                    <dateSingle>
                                        <textDate><xsl:value-of select="regex-group(0)"/></textDate>
                                        <standardDate>
                                            <xsl:value-of select="fn:replace(regex-group(0), '[^\d+]', '')"/>
                                        </standardDate>
